...
export const seatalkPlugin: ChannelPlugin<ResolvedSeaTalkAccount> = {
  ...
  config: {
    ...
    resolveAccount: (cfg, accountId) =>
      resolveSeaTalkAccount({ cfg, accountId }),
    ...
    deleteAccount: ({ cfg, accountId }) => {
      ...
      if (isDefault) {
        ...
        const hasOtherChannels = Object.values(nextChannels).some(
          (v) => v !== undefined
        );
    ...
    resolveTargets: async ({ cfg, accountId, inputs }) => {
      ...
      return inputs.map((input) => {
        ...
        return {
          input,
          resolved: false,
          note: "No active employee found for this email",
        };
      });
    },
  },
  ...
  gateway: {
    startAccount: async (ctx) => {
      const account = resolveSeaTalkAccount({
        cfg: ctx.cfg,
