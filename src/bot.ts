...
async function issueSeaTalkPairingChallenge(params: {
  ...
  const { code, created } =
    await params.core.channel.pairing.upsertPairingRequest({
      channel: "seatalk",
...
export function dispatchSeaTalkEvent(params: {
  ...
  const handle = (fn: () => Promise<void>) =>
    fn().catch((err) =>
      error(`seatalk[${accountId}]: event error: ${String(err)}`)
    );

  switch (event.event_type) {
    case "message_from_bot_subscriber":
      handle(() =>
        handleSeaTalkMessage({ cfg, event, client, runtime, accountId })
      );
      break;
    case "new_mentioned_message_received_from_group_chat":
    case "new_message_received_from_thread":
      handle(() =>
        handleSeaTalkGroupMessage({ cfg, event, client, runtime, accountId })
      );
      break;
...
function flushBuffer(key: string): void {
  ...
  if (first.kind === "dm") {
    ...
    processBufferedGroupEvents(groupEntries, state.context).catch((err) => {
      const error = state.context.runtime?.error ?? console.error;
      error(
        `seatalk[${state.context.accountId}]: group flush error: ${String(err)}`
      );
    });
  }
}
...
async function resolveQuotedMessage(params: {
  ...
  try {
    ...
    const sender =
      (data.sender as { employee_code?: string } | undefined)?.employee_code ??
      "unknown";
    ...
    if (tag === "text") {
      const textObj = data.text as
        | { plain_text?: string; content?: string }
        | undefined;
      ...
      const fakeMsg: SeaTalkMessage = {
        ...
        image:
          tag === "image"
            ? (data.image as { content: string } | undefined)
            : undefined,
        file:
          tag === "file"
        ...
        video:
          tag === "video"
            ? (data.video as { content: string } | undefined)
            : undefined,
      };
    ...
    log(
      `seatalk: failed to resolve quoted message ${quotedMessageId}: ${String(
        err
      )}`
    );
    return null;
...
async function processBufferedDmEvents(
  ...
  if (accessDecision.decision !== "allow") {
    if (accessDecision.reason === "not allowlisted") {
      log(
        `seatalk[${accountId}]: sender ${employeeCode} not in allowlist, dropping`
      );
    } else {
      log(
  ...
  try {
    ...
    core.system.enqueueSystemEvent(
      `SeaTalk[${accountId}] DM from ${senderName}: ${preview}`,
      {
    ...
    if (processingIndicator === "typing") {
      client
        ...
        .catch((err) =>
          log(`seatalk[${accountId}]: typing failed: ${String(err)}`)
        );
    }
    ...
    log(
      `seatalk[${accountId}]: dispatching to agent (session=${route.sessionKey})`
    );
...
export async function handleSeaTalkMessage(params: {
  ...
  const key = dmDebounceKey(
    accountId,
    msgEvent.employee_code,
    msgEvent.message.thread_id
  );
  pushToBuffer(
    key,
...
function checkGroupAccess(params: {
  ...
  if (groupSenderAllowFrom && groupSenderAllowFrom.length > 0) {
    const match = groupSenderAllowFrom.some((entry) => {
      ...
      if (senderEmail && e.toLowerCase() === senderEmail.toLowerCase())
        return true;
      return false;
    });
    if (!match) {
...
export async function handleSeaTalkGroupMessage(params: {
  ...
  if (!tryRecordEvent(`${accountId}:${event.event_id}`)) {
    log(
      `seatalk[${accountId}]: skipping duplicate group event ${event.event_id}`
    );
    return;
  }
  ...
  try {
    ...
    if (processingIndicator === "typing") {
      client
        ...
        .catch((err) =>
          log(`seatalk[${accountId}]: group typing failed: ${String(err)}`)
        );
    }
    ...
    const typingResult = core.channel.reply.createReplyDispatcherWithTyping({
      ...
      onError: (err) => {
        error(
          `seatalk[${accountId}]: group reply delivery failed: ${String(err)}`
        );
      },
    });
    ...
    log(
      `seatalk[${accountId}]: dispatching group message (session=${route.sessionKey})`
    );
    ...
    error(
      `seatalk[${accountId}]: failed to dispatch group message: ${String(err)}`
    );
  }
}
